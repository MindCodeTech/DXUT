...
{
    XMFLOAT4 vMaterialColor;                     // mesh color
    XMFLOAT4 vAmbientColor;                      // mesh ambient color
    XMFLOAT4 vSpecularColor;                     // mesh specular highlight color
    XMFLOAT4 vScreenSize;                        // x,y = screen width,height
    XMFLOAT4 vFlags;                             // miscellaneous flags
};

// constants that need to be updated every frame
struct UPDATE_CB_STRUCT
{
     XMFLOAT4X4A mWorld;                         // World matrix
     XMFLOAT4X4A mViewProjection;                // View-Projection matrix
     XMFLOAT4X4A mWorldViewProjection;           // WVP matrix
    XMFLOAT4 vTessellationFactor;              // Edge, inside and minimum tessellation factor
    XMFLOAT4 vDisplacementScaleBias;           // Scale and bias of displacement
    XMFLOAT4 vLightPosition;                   // 3D light position
    XMFLOAT4 vEyePosition;                     // 3D eye position
};
...
{
    XMFLOAT4 vNormal[MAX_DECALS];
    XMFLOAT4 vBinormal[MAX_DECALS];
    XMFLOAT4 vTangent[MAX_DECALS];
    XMFLOAT4 vDecalPositonSize[MAX_DECALS];
};
...
MESH_TYPE                           g_eMeshType = MESH_TYPE_CAR;
 XMFLOAT4X4A                          g_m4x4MeshMatrix[MESH_TYPE_MAX];
 XMFLOAT4X4A                          g_m4x4MeshMatrixInv[MESH_TYPE_MAX];
float                               g_fDecalRadiusScale[MESH_TYPE_MAX];
float                               g_fMaxTessellationFactor[MESH_TYPE_MAX];
XMFLOAT4                         g_vMeshColor[MESH_TYPE_MAX];
XMFLOAT4                         g_vMeshSpecularColor[MESH_TYPE_MAX];
XMFLOAT4                         g_vAmbientColor;
...
XMFLOAT4                         g_vScreenSize;                      // curent screen width and height
...
void RenderText()
{
    ...
    g_pTxtHelper->SetForegroundColor( XMCOLOR( 1.0f, 1.0f, 0.0f, 1.0f ) );
                                      ...
                                      void* pUserContext )
{
    ...
    XMFLOAT3 vecEye( 0.0f, 0.0f, 0.0f );
    ...
    // Setup the matrix for each mesh
     XMFLOAT4X4A mModelRotationX;
     XMFLOAT4X4A mModelRotationY;
     XMFLOAT4X4A mModelTranslation;
                                  ...
                                  double fTime, float fElapsedTime, void* pUserContext )
{
    ...
    const XMFLOAT3* eyePt = g_Camera[g_eMeshType].GetEyePt();
    XMFLOAT4 vEyePt;
    vEyePt.x = eyePt->x;
    vEyePt.y = eyePt->y;
    vEyePt.z = eyePt->z;
    ...
    if( g_Shoot )
    {
        ...
        if( RayCastForDecalPosition( bulletRay, g_SceneModel[g_eMeshType], &intersection, &pTri ) )
        {
            ...
            for( UINT i = 0; i < MAX_DECALS; i++ )
            {
                ...
                XMFLOAT3 vIntToDecal = decalLocation - intersection;
            ...
            XMFLOAT4 vNormalConst;
            XMFLOAT4 vBinormalConst;
            XMFLOAT4 vTangentConst;
            XMFLOAT4 vDecalPositionSize;
    ...
    update_cb_struct.vTessellationFactor = 
        XMFLOAT4( g_TessellationFactor, g_bScreenSpaceAdaptive, g_bBackFaceCulling, g_bDisplacementAdaptive );
    update_cb_struct.vDisplacementScaleBias = 
        XMFLOAT4( scale * g_fDecalRadiusScale[g_eMeshType], bias * g_fDecalRadiusScale[g_eMeshType], 
