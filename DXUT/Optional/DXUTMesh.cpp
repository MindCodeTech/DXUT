...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDXUTMesh::Create(ID3D11Device* pd3dDevice, LPCWSTR strFilename)
{
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDXUTMesh::Create(ID3D11Device* pd3dDevice,
...
HRESULT CDXUTMesh::CreateMaterials(LPCWSTR strPath, ID3D11Device *pd3dDevice, ID3D11Buffer *pAdjacencyBuffer, ID3D11Buffer *pMtrlBuffer)
{
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDXUTMesh::SetFVF(ID3D11Device* pd3dDevice, DWORD dwFVF)
{
...
//       declarations.
//-----------------------------------------------------------------------------
HRESULT CDXUTMesh::SetVertexDecl(ID3D11Device* pd3dDevice, const D3DVERTEXELEMENT9 *pDecl)
{
	LPD3DXMESH pTempSysMemMesh = NULL;
	LPD3DXMESH pTempLocalMesh = NULL;
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDXUTMesh::RestoreDeviceObjects(ID3D11Device* pd3dDevice)
{
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDXUTMesh::Render(ID3DX11Effect *pEffect,
	D3DXHANDLE hTexture,
	D3DXHANDLE hDiffuse,
	D3DXHANDLE hAmbient,
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDXUTMeshFrame::RestoreDeviceObjects(ID3D11Device* pd3dDevice)
{
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDXUTMeshFrame::Render(ID3D11Device* pd3dDevice, bool bDrawOpaqueSubsets,
	bool bDrawAlphaSubsets, XMMATRIX* pmatWorldMatrix)
{
	...
	XMMATRIX matSavedWorld, matWorld;
	...
	XMMatrixMultiply(&matWorld, &m_mat, &matSavedWorld);
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDXUTMeshFile::LoadFrame(ID3D11Device* pd3dDevice,
	LPD3DXFILEDATA pFileData,
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDXUTMeshFile::LoadMesh(ID3D11Device* pd3dDevice,
	LPD3DXFILEDATA pFileData,
	CDXUTMeshFrame* pParentFrame)
{
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDXUTMeshFile::CreateFromResource(ID3D11Device* pd3dDevice, LPCWSTR strResource, LPCWSTR strType)
{
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDXUTMeshFile::Create(ID3D11Device* pd3dDevice, LPCWSTR strFilename)
{
	LPD3DXFILE           pDXFile = NULL;
	LPD3DXFILEENUMOBJECT pEnumObj = NULL;
	LPD3DXFILEDATA       pFileData = NULL;
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDXUTMeshFile::Render(ID3D11Device* pd3dDevice, XMMATRIX* pmatWorldMatrix)
{
	// For pure devices, specify the world transform. If the world transform is not
	// specified on pure devices, this function will fail.
	...
	XMMatrixMultiply(&matWorld, &matSavedWorld, &m_mat);
	pd3dDevice->SetTransform(D3DTS_WORLD, &matWorld);
